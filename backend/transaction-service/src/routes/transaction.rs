...
/// - `GET /:id` - Get a specific transaction (protected by auth middleware)
...
async fn get_wallet_transactions(
    ...
    let transactions = database::transaction::find_by_wallet(wallet_id, user_id, pool)
        .await
        .inspect_err(|_| {
            tracing::error!(
    ...
    tracing::info!(
        "Successfully fetched {} transactions for wallet {}",
        transactions.len(),
        wallet_id
    );
    Ok(Json(transactions))
...
async fn update_transaction(
    ...
    tracing::debug!(
        "Update payload: amount={:?}, type={:?}, category={:?}",
