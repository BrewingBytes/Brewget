...
<template>
  <GlassDialog :visible="visible" @update:visible="emit('update:visible', $event)"
    ...
    <template #footer>
      <Button :label="t('transactions.cancel')" icon="pi pi-times" text @click="emit('update:visible', false)"
        class="text-white! hover:bg-white/10!" />
      <Button :label="t('transactions.delete_transaction')" icon="pi pi-trash" severity="danger" @click="handleDelete"
