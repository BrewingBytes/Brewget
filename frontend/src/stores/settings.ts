...
export const useSettingsStore = defineStore("settings", () => {
  ...
  async function loadSettings(): Promise<void> {
    ...
    try {
      ...
      if (response.status !== ServerStatus.NO_ERROR) {
        useToastStore().showError(t("settings.failed_to_load"));
        return;
      }
  ...
  async function updateSettings(updates: UpdateSettings): Promise<boolean> {
    ...
    try {
      ...
      if (response.status !== ServerStatus.NO_ERROR) {
        useToastStore().showError(t("settings.failed_to_update"));
        return false;
      }

      settings.value = response.data;
      useToastStore().showInfo(t("settings.settings_updated"));
      return true;
    } finally {
      loading.value = false;
    }
